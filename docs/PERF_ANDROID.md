...
`docs/BIND.md` §7 은 바인딩 플랜(최적화 1)을 기기에서 쟀지만(view 2.75x, transpose 2.51x,
add 1.80x), 같은 세션에 착지한 나머지 넷 — 디스패치 키워드 관례(`docs/DISPATCH.md`),
`pow` 를 곱셈으로 바꾼 것(`docs/SEQLEN.md` §2-3), 전치를 통과하는 bf16 GEMM 확장
(`docs/DTYPE_PERF.md`), `amax`·스케일-마스크 융합·5.25x 어텐션 복사(`docs/SEQLEN.md` §7-8)
...
`include_str!` 로 `.so` 에 굳어 있으므로(`docs/BIND.md` §7.1 이 이미 기록) 이 세션에서도
**`_C.abi3.so` 하나만 바꿔치기하면 버전을 전환할 수 있다.** CPython 런타임과 의존성은
`scripts/device_android.sh stage` 가 이전 세션에 이미 올려둔 것을 그대로 재사용했다.

방법은 `docs/BIND.md` §7.2 를 따른다: old/new 를 번갈아 3 회, 각 회차는 워밍업 후 최솟값,
...
## 10.1 최적화 #2 — 디스패치 키워드 관례 (`docs/DISPATCH.md`, `e39c65e`)

휠 `0.0.3a0`(old, 키워드 언패킹 경로) 대 `0.0.4a0`(new, `interned_name` 인터닝 경로).
`docs/BIND.md` §7.2 와 동일한 모양 `(1,6,9,64)` f32, 워밍업 200, 2 만 호출씩 5 블록의 최솟값.
3 회 번갈아, µs/call:
...
unsqueeze 1.0317×. 대조군이 최대 3.2% 까지 흔들려 `docs/BIND.md` 의 대조군(1.03~1.04x)보다
조금 더 넓지만, 관측된 효과(11~19%)가 그보다 여전히 3.5~12 배 커서 잡음으로 설명되지 않는다.

**전이됐다, 그리고 호스트보다 크다.** 호스트(`docs/DISPATCH.md`): view 1.100×, transpose
1.153×, add 1.073×, unsqueeze 1.118×. 기기: view/transpose 1.19×, add 1.11×, unsqueeze 1.17× —
네 지표 모두 호스트보다 크거나 같다. `docs/BIND.md` §7 이 예상한 대로("느린 인터프리터가
파이썬 층의 몫을 키운다") 다.

## 10.2 최적화 #3 — `pow` 를 곱셈으로 (`docs/SEQLEN.md` §2-3, `59935ef`)
...
**전이됐고, 미시 벤치 자체의 배수가 호스트보다 크다.** 호스트(`docs/SEQLEN.md` §2.3, S=512)의
...
## 10.3 최적화 #4 — bf16 GEMM 확장을 전치 뒤로 (`docs/DTYPE_PERF.md`, `c8ed26f`)
...
(`docs/DTYPE_PERF.md` 결론부, SmolLM2-135M `S=128` 전체 prefill 469.4→180.7ms, 커밋 메시지는
...
## 10.4 최적화 #5 — `amax`, 스케일-마스크 융합, 5.25x 어텐션 복사 (`docs/SEQLEN.md` §7-8)
...
있음으로 바뀐 것 자체가 결과다. SDPA 전체로 보면 **1.11×** 개선. 호스트(`docs/SEQLEN.md`
§7.6, `S=512`, 상류 대비): 2.007×→1.684×, old/new 로 다시 읽으면 463.73/389.01=**1.19×**.
**여기서는 기기(1.11×)가 호스트(1.19×)보다 작다** — `docs/BIND.md` §7 의 "느린 인터프리터가
...
**복사가 전이됐지만 5.25× 가 아니라 ~3.1~3.7× 다.** 호스트(`docs/SEQLEN.md` §8.3/8.6):
...
|---|---:|---:|---|
| #1 바인딩 플랜(`docs/BIND.md`, 기존 측정) | view 2.91×, transpose 2.77× | view 2.75×, transpose 2.51×, add 1.80× | 전이됨 |
...
`docs/BIND.md` §7 의 주장은 최적화 #4(bf16 GEMM), #5a(`amax`), #5b/c(복사 단독)에서 성립하지
