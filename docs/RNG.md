...
### 3.3 다만 `normal_` 의 **비트 단위** 일치는 libm 에 걸린다

`uniform_` 은 정수 마스크와 곱셈뿐이라 비트 단위로 정확합니다. `normal_` 은
`sqrt`/`log`/`cos`/`sin` 을 **`opmath_t` 정밀도로** 부르므로(`NormalFill16::operator()`,
`DistributionTemplates.h:100-108`), f32 경로는 플랫폼 libm 의 `logf`/`cosf`/`sinf` 반올림에
의존합니다. 파이썬 포팅(double 로 계산 후 1 회 반올림 = 정확 반올림)과 torch 실측을 5,120 개
비교한 ulp 분포:
...
`std::fma` 사용 여부는 이 결과를 바꾸지 않았습니다(fma 유/무 두 구현 모두 같은 비트 수가 일치).
차이의 원인은 아핀이 아니라 **초월함수 반올림**입니다.

**의미**: `normal_` 을 골든 하네스에서 `expect="match"` 로 비교하는 것은 가능하지만, **비트 동일이
아니라 float32 허용오차(~1e-6 상대) 기준**이어야 합니다. 이는 `tools/golden/compare.py:211` 의
`dt.tolerance_for(t_dtype)` 경로가 이미 하는 일이므로 하네스 구조를 바꿀 필요는 없습니다.
**"비트 단위로 같아야 한다" 는 기준을 세우면 안 됩니다** — 그것은 우리 구현의 버그가 아니라
libm 차이에서 실패합니다.
...
**3. 골든 하네스의 기대치를 op 별로 나눈다.**

| op | `expect` | 비교 기준 |
|---|---|---|
| `aten.uniform_.default` | `match` | **비트 단위** 가능 |
| `aten.normal_.default` | `match` | float32 허용오차 (**비트 단위 금지** — libm 차이로 최악 3 ulp, §3.3) |
| `aten.randint.low` | 현행 `_range_check` 유지 | 알고리즘 재현이 **미확인**이므로 승격은 그 확인 이후 |
