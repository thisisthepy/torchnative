...
│                    수명 이름은 ttadapters 의 ScenarioType 에 맞춤
...
**수명은 방법이 아니라 정책입니다.** 같은 TENT 를 episodic 으로 돌릴지 continual 로 돌릴지는
방법이 아니라 시나리오가 정합니다. 그리고 이 축에는 이 프로젝트에 **이미 이름이 있습니다** —
`ttadapters/datasets/scenarios/base.py` 의 `ScenarioType`:

| `ScenarioType` | 수명 |
|---|---|
| `STANDARD` | 도메인마다 베이스 상태로 리셋 |
| `GRADUAL` | 점진적 분포 변화를 따라감 |
| `CONTINUAL` | 리셋 없이 도메인 시퀀스를 누적 |
| `UNIVERSAL` | 도메인 경계를 가정하지 않음 |

**`delta/` 의 수명 정책은 새 이름을 만들지 말고 여기에 맞춥니다.** 아키텍처로서의 TTT 는 이보다
한 단계 짧은 수명(시퀀스 하나)이 추가로 필요합니다.
...
이들을 관통하는 것은 **베이스 가중치 위의 델타**입니다 — TTT 의 fast weight, TTA 의 적응된
파라미터, FL 의 로컬 업데이트가 전부 같은 물건이고 **수명과 행선지만 다릅니다.**
