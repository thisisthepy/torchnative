...
> **갱신 (docs/ADAPT.md, 2026-09) — 첫 실사용처가 생겼고, 이름은 필요하지 않았습니다.**
...
>
> > **정정 (2026-09-02, `docs/FEDERATED.md`).** 둘 다 거절을 멈췄습니다. `persist()` 는
> > `docs/BACKWARD.md` §14 에서, `publish()` 는 `world_size = 2` 전송(`docs/TRANSPORT.md`) 위에서.
...
> 35,136 개 중 2 개가 어긋납니다(`(w+d)-d ≠ w`). 측정치는 docs/ADAPT.md §5.
...
> **구현됨 (docs/ADAPT.md §2).** `torchnative.delta.Delta` 가 베이스 · 오프셋 · 세 수명 질문을
...
> 없습니다. `docs/WASM.md` 가 네 층을 다 재서 그 판단의 근거를 담고 있습니다 — candle 은
...
**순서와 무관하게 지금 가장 큰 것입니다.** `docs/QUANT2.md` §6.4 가 양자화를 재다가 찾았고,
